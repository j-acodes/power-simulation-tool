...
- [x] A valid PV-only diagram meets its active-power target and requested POC
      reactive duty within explicit numerical tolerances.
- [x] A valid BESS-only diagram meets the same active/reactive requirements
      without changing container, delivered-energy, duration, or auxiliary-load
      semantics.
- [x] The known 3 MW at 0.95 power-factor regression no longer returns the
      previously understated reactive delivery.
- [x] P, Q, apparent power, effective power factor, station loading/current,
...
- [x] Auto-selected cables are reselected until stable when convergence changes
      their required capacity; forced choices that become invalid return a
      structured engine issue with no partial result.
- [x] A case that crosses a current, loading, utilization, or loss threshold
      only after refinement is classified using the converged value.
- [x] Convergence is bounded and deterministic; failure returns the existing
      issue/results envelope rather than an uncaught exception or stale result.
- [x] Existing single-fleet diagrams and API contracts remain backward
