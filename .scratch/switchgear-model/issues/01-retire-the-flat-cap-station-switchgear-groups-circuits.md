...
- [ ] Stage-1 grouping admits a circuit only when every position's accumulated through current is within that position's station switchgear rated current; the `test_assign_*` family asserts that, not scalar-cap semantics
- [ ] `max_circuit_current_a` no longer exists in engine, backend, API, frontend, seed or PDF; a diagram carrying it in `settings.rules` solves identically to one without it
- [ ] Before `tests/golden_baseline.json` is regenerated, one design's circuit count and trunk through current are computed by hand and pinned as an explicit test that passes
- [ ] Golden baseline regenerated after the anchor passes; the commit message explains why the numbers moved
- [ ] `.venv/bin/python -m pytest -q`, frontend Vitest, `npm run build` and `npm run lint` pass
- [ ] Report states that a DB reset is required at ship time (settings key removed); do not run `scripts/reset_db.py`
